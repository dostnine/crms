...
const pstoIcons = [
    'ri-map-pin-2-line', 'ri-map-pin-user-line', 'ri-government-line', 'ri-building-4-line',
    'ri-compass-3-line', 'ri-road-map-line', 'ri-navigation-line', 'ri-map-2-line',
    'ri-pin-distance-line', 'ri-earth-line',
];
const iconFor = (i) => pstoIcons[i % pstoIcons.length];

const goBack = () => window.history.back();
</script>

<template>
    <Head title="Select PSTO" />
        ...
        <main class="rmain">
            <header class="rhero">
                <span class="rhero-kicker">
                    ...
                    <span v-else>Provincial Offices</span>
                </span>
                <h1 class="rhero-title">Select a <span class="gradient">PSTO</span></h1>
                <p class="rhero-sub">Choose the provincial S&amp;T office you transacted with.</p>
                <span class="rhero-count">{{ pstos?.length || 0 }} offices available</span>
            </header>

            <div class="card-grid">
                <Link
                    v-for="(psto, index) in pstos"
                    :key="psto.id"
                    :href="'/services/csf?region_id=' + region_id + '&service_id=' + service_id + '&unit_id=' + unit_id + '&sub_unit_id=' + sub_unit_id + '&psto_id=' + psto.id"
                    class="flow-card"
                    :style="{ animationDelay: (index * 45) + 'ms' }"
                >
                    <div class="flow-icon"><i :class="iconFor(index)"></i></div>
                    <h3 class="flow-name">{{ psto.psto_name }}</h3>
